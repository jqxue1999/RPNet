    ...
    def simba_batch(self, images_batch, labels_batch, max_iters, freq_dims, stride, epsilon, linf_bound=0.0,
        ...
        for k in range(max_iters):
            ...
            if queries_num >= 1000 and res[1000] == -1:
                ...
                break
            elif queries_num >= 500 and res[500] == -1:
                res[500] = rate
                print("queries={}, rate={:.4f}".format(queries_num, rate))
            elif queries_num >= 400 and res[400] == -1:
                res[400] = rate
                print("queries={}, rate={:.4f}".format(queries_num, rate))
            elif queries_num >= 300 and res[300] == -1:
                res[300] = rate
                print("queries={}, rate={:.4f}".format(queries_num, rate))
            elif queries_num >= 200 and res[200] == -1:
                res[200] = rate
                print("queries={}, rate={:.4f}".format(queries_num, rate))
            elif queries_num >= 100 and res[100] == -1:
                res[100] = rate
                print("queries={}, rate={:.4f}".format(queries_num, rate))
            else:
                continue
